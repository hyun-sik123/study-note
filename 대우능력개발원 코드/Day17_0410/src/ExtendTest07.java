/* 상속에서 생성자 호출문제 에러를 막는 가장 합리적인 방법은 부모의 생성자가 오버로딩이 되었을 때 기본생성자 묵시적 제공을 하지 않은 상태
 * 에서 명시적으로 부모의 기본생성자를 코드를 하는 것 보다는 자손에서 부모의 오버로딩 된 생성자를 호출하도록 경로를 변경하는 것이 가장
 * 좋다. 
 */
